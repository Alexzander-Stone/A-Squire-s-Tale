...
func update(delta):
	...
	if(horizontalMove != 0 and verticalMove != 0):
		...
		owner.velocity.x = cos(playerAngle) * horizontalMove * owner.MAX_VELOCITY
		owner.velocity.y = sin(playerAngle) * verticalMove * owner.MAX_VELOCITY
	else:
		owner.velocity.x = horizontalMove * owner.MAX_VELOCITY 
		owner.velocity.y = verticalMove * owner.MAX_VELOCITY 
